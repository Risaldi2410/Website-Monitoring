...
if (isset($_POST['simpan'])) {
	...
	$queryUser = mysqli_query($conn, "SELECT * FROM tbl_user WHERE username = '$username'");
	...
	if (!password_verify($curPass, $data['password'])) {
		...
		mysqli_query($conn, "UPDATE tbl_user SET password = '$pass' WHERE username ='$username'");
		header("location:password.php?msg=update");
		exit;
	}
}
